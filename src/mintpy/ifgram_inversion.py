...
def _ref_index_for_full_network(date_list, ref_date):
    """Return the column index of REF_DATE in a complete date list (0 if unset)."""
    if not date_list or not ref_date:
        return 0
    if ref_date not in date_list:
        raise ValueError(f'reference date {ref_date} is not in date_list!')
    return date_list.index(ref_date)


def _rereference_full_network_ts(ts, date_list, ref_date):
    """Shift a first-date-referenced timeseries so that REF_DATE is zero.

    The vectorized SBAS path always reconstructs with ts[0]=0. When the
    inversion REF_DATE is not the first acquisition, subtract that date.
    """
    ref_ind = _ref_index_for_full_network(date_list, ref_date)
    if ref_ind != 0:
        ts -= ts[ref_ind:ref_ind + 1, :]
    return ts


def _fill_full_network_phase_ts(ts, X, date_list, ref_date):
    """Write min-norm-phase solution X into ts with 0 at REF_DATE.

    X columns follow the design matrix A, which omits REF_DATE rather than
    always omitting the first date.
    """
    ref_ind = _ref_index_for_full_network(date_list, ref_date)
    if ref_ind == 0:
        ts[1:, :] = X
    else:
        non_ref = [i for i in range(ts.shape[0]) if i != ref_ind]
        ts[non_ref, :] = X
    return ts
...
def estimate_timeseries(A, B, y, tbase_diff, weight_sqrt=None, min_norm_velocity=True,
    ...
    try:
        if min_norm_velocity:
            ...
            _rereference_full_network_ts(ts, date_list, ref_date)
            ...
            _fill_full_network_phase_ts(ts, X, date_list, ref_date)
...
def run_ifgram_inversion_patch(ifgram_file, box=None, ref_phase=None, obs_ds_name='unwrapPhase',
    ...
    # full-network pixels stay on the vectorized path; partial-network options
    # are only used for pixels that miss some interferograms / dates
    kwargs_full = {
        'A'                     : A,
        'B'                     : B,
        ...
        'inv_quality_name'      : inv_quality_name,
        'allow_partial_network' : False,
        # still needed when refDate is not the first date
        'date_list'             : date_list,
        'ref_date'              : ref_date,
    }
    kwargs_part = dict(kwargs_full)
    kwargs_part.update({
    ...
    # 2.3 weighted inversion - pixel-by-pixel
        ...
        kwargs_wls = kwargs_part if allow_partial_network else kwargs_full
